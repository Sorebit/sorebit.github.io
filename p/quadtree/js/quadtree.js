...
function Quadtree(bounds, maxObjects, maxLevels, level) {
	...
	self.split = function() {
		...
		var x = Math.floor(self.bounds.x);
		var y = Math.floor(self.bounds.y);
		var nextLevel = self.level + 1;

		self.nodes[0] = new Quadtree(new Rectangle(x + subWidth, y, subWidth, subHeight), self.maxObjects, self.maxLevels, nextLevel);
		self.nodes[1] = new Quadtree(new Rectangle(x, y, subWidth, subHeight), self.maxObjects, self.maxLevels, nextLevel);
		self.nodes[2] = new Quadtree(new Rectangle(x, y + subHeight, subWidth, subHeight), self.maxObjects, self.maxLevels, nextLevel);
		self.nodes[3] = new Quadtree(new Rectangle(x + subWidth, y + subHeight, subWidth, subHeight), self.maxObjects, self.maxLevels, nextLevel);
	}
